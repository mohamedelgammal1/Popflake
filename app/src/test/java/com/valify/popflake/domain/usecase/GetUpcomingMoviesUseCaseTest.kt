...
class GetUpcomingMoviesUseCaseTest {
    ...
    fun `invoke returns success result with movies`() = runBlocking {
        ...
        val movies = listOf(
            Movie(id = 1, title = "Movie 1", overview = "Overview 1", posterPath = "/path1.jpg",
                backdropPath = "/backdrop1.jpg", releaseDate = "2025-01-01", voteAverage = 0.0),
            Movie(id = 2, title = "Movie 2", overview = "Overview 2", posterPath = "/path2.jpg",
                backdropPath = "/backdrop2.jpg", releaseDate = "2025-06-02", voteAverage = 0.0)
        )
        coEvery { movieRepository.getUpcomingMovies(any()) } returns flowOf(Result.success(movies))

        // Act
        val result = getUpcomingMoviesUseCase(forceRefresh = false).first()
    ...
    fun `invoke returns error result when repository fails`() = runBlocking {
        ...
        coEvery { movieRepository.getUpcomingMovies(any()) } returns flowOf(Result.failure(exception))

        // Act
        val result = getUpcomingMoviesUseCase(forceRefresh = true).first()
